...
nodes = []
File.read(input).lines[2..-1].each do |line|
  x, y, size, used, avail = /x(\d+)-y(\d+)\s+(\d+)T\s+(\d+)T\s+(\d+)T/.match(line)[1..-1].map(&:to_i)
  nodes << Node.new(x, y, size, used, avail)
end

viable_nodes = []
n1 = 0
while n1 < nodes.length
  n2 = 0
  while n2 < nodes.length
    viable_nodes << [n1,n2] if n1 != n2 && nodes[n1].used != 0 && nodes[n1].used <= nodes[n2].avail
    n2 += 1
  end
  n1 += 1
end
...
# Part 2
# No idea how to solve this using code, so let's print the grid
# and solve it by hand
def print_nodes(nodes)
  node_grid = []
  max_size = nodes[0].size
  0..39.times { |i| node_grid[i] = [] }
  nodes.each do |n|
    if n.x == 0 && n.y == 0
      node_grid[n.x][n.y] = "0"
    elsif n.x == 38 && n.y == 0
  ...
  node_grid = node_grid.transpose

  x = 0
  while x < node_grid.length
    y = 0
    while y < node_grid[x].length
      print node_grid[x][y]
      y += 1
    end
    puts
...
puts "Part 1: #{viable_nodes.length}"
print_nodes(nodes)
...
# .......................................
#
# _ is the empty node. Need to shuffle this along around the wall of large, full
# nodes, up to the top right corner, and finally shuffle the G leftwards so it is
# at position 0.
#
# _ moves as follows:
# 8 left
...
# 32 right
#
# Shuffling the G takes 5 moves to shift it 1 place to the left
# 37 * 5 = 185 to move G next to 0
# +1 to finally move G into 0
# 8 + 23 + 32 + 185 + 1 = 249
puts "Part 2: #{249}"
