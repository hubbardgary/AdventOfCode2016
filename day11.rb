...
class Building
  attr_accessor :elevator_position, :floors, :moves
  def initialize(building)
    if building == nil
      ...
      @elevator_position = building.elevator_position
      @floors = { 1 => Array.new(building.floors[1]),
                  2 => Array.new(building.floors[2]),
                  3 => Array.new(building.floors[3]),
                  4 => Array.new(building.floors[4]) }
      @moves = building.moves
    end
  end

  def valid?
    @floors.each do |i,f|
      chips = (f.select { |c| c.end_with?('m') }).map { |c| c.chomp('m') }
      generators = (f.select { |g| g.end_with?('g') }).map { |g| g.chomp('g') }

      # If any chips are on the same floor as any unattached generated, they fry.
      return false if (chips - generators).any? && (generators - chips).any?
    end
    true
  end
end

def get_state_hash(building)
  # Optimisation, with thanks to p_tseng https://www.reddit.com/r/adventofcode/comments/5hoia9/2016_day_11_solutions/db1v1ws/
  # All pairs are interchangeable, so create hash of positions
  # of related items rather than the position of all specific items.
  # The keys hash determines the number to substitute element name for.
  # 'Optimisation' is used fairly loosely - this is still horribly inefficient!
  keys = {}
  hash = "elevator=#{building.elevator_position}"
  building.floors.each do |i,floor|
    hash += "##{i.to_s}-"
    floor.sort.each do |item|
      chars = item.chars
      # If we haven't come across this element before, add a mapping for it
      keys[chars.first] = keys.length + 1 if !keys.key?(chars.first)
      chars[0] = keys[chars.first]
      hash += "#{chars.join}|"
    end
    hash.chop! # remove trailing pipe
  end
  hash
end

def get_next_state_combinations(current_floor)
  # Next states are all groups of 1 or 2 items
  combinations = [current_floor.combination(2).to_a]
  combinations << current_floor.combination(1).to_a
  combinations.flatten!(1)
end

def get_states_for_next_floor(building, item_combinations, current_floor, next_floor)
  states = []
  item_combinations.each do |com|
    state = Building.new(building)
    com.each do |item|
      state.floors[next_floor] << item
      state.floors[current_floor].delete(item)
    end
    state.floors[next_floor].sort!
    state.elevator_position = next_floor
    state_hash = get_state_hash(state)

    if state.valid? && !$previous_states.include?(state_hash)
      $previous_states << state_hash
      states << state
    end
  end
  states
...
def next_states(building)
  combinations = get_next_state_combinations(building.floors[building.elevator_position])
  states = []

  # We can go up and down a floor, unless we're on the top or bottom floor
  if building.elevator_position < building.floors.length
    states += get_states_for_next_floor(building, combinations, building.elevator_position, building.elevator_position + 1)
  end
  if building.elevator_position > 1
    states += get_states_for_next_floor(building, combinations, building.elevator_position, building.elevator_position - 1)
  end
  states
end

def all_on_top_floor(building)
  (1..building.floors.length - 1).each { |i| return false if building.floors[i].any? }
  true
end

def print_building(building)
  puts "F4 #{building.floors[4]}"
  puts "F3 #{building.floors[3]}"
  puts "F2 #{building.floors[2]}"
  puts "F1 #{building.floors[1]}"
  puts "Elevator is on floor #{building.elevator_position}"
end
...
def breadth_first_search(initial_state)
  ...
  while queue.length > 0
    current = queue.shift
    if all_on_top_floor(current)
      puts current.inspect
      print_building(current)
      return current.moves
    end
    current.moves += 1
    next_states(current).each { |s| queue << s }
  end
end

def get_start_state(input)
  building = Building.new(nil)
  floor = 1
  File.read(input).lines.each do |line|
    line.scan(/\w+-compatible microchip|\w+ generator/).each do |item|
      building.floors[floor] << "#{item.chars.first}m" if item.end_with?('microchip')
      building.floors[floor] << "#{item.chars.first}g" if item.end_with?('generator')
      building.floors[floor].sort!
    end
    floor += 1
  end
  building
...
building = get_start_state(input)
$previous_states << get_state_hash(building)
part1 = breadth_first_search(building)
puts "Part 1: #{part1}"

# Part 2
building.floors[1] << 'em'
building.floors[1] << 'eg'
building.floors[1] << 'dm'
building.floors[1] << 'dg'
building.moves = 0
$previous_states = Set.new
$previous_states << get_state_hash(building)
part2 = breadth_first_search(building)
