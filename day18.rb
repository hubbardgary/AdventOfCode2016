...
def get_tile(previous_row, i)
  left = i == 0 ? '.' : previous_row[i-1]
  middle = previous_row[i]
  right = i == previous_row.length-1 ? '.' : previous_row[i+1]
...
while rows.length < 400000
  new_row = []
  previous_row = rows[-1].chars

  i = 0
  while new_row.length < rows[-1].length
    new_row << get_tile(previous_row, i)
    i += 1
  end
  rows << new_row.join
  puts "Part 1: #{rows.join.count('.')}" if rows.length == 40
end

puts "Part 2: #{rows.join.count('.')}"
