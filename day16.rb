...
disk_length_part1 = 272
disk_length_part2 = 35651584

def generate_data(data, disk_length)
  return data[0, disk_length] if data.length >= disk_length

  a = data
  b = data.reverse.tr('10', '01')
  new_data = "#{a}0#{b}"
  return generate_data(new_data, disk_length)
end

def generate_checksum(data)
  return data if data.length.odd?
  checksum = []
  data.scan(/../).each { |e| if e[0] == e[1] then checksum << '1' else checksum << '0' end }
  return generate_checksum(checksum.join)
end

puts "Part 1: #{generate_checksum(generate_data(input, disk_length_part1))}"
puts "Part 2: #{generate_checksum(generate_data(input, disk_length_part2))}"
