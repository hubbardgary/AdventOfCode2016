...
def add_rect(width, height)
  height.times { |h| width.times { |w| $screen[h][w] = '#' }}
end

initialise_screen()

File.read(input).lines.each do |instruction|
  if instruction.start_with?('rect')
    size = instruction.scan(/\d+/)
    add_rect(size[0].to_i, size[1].to_i)
  elsif instruction.start_with?('rotate row')
    row, distance = instruction.scan(/\d+/).map { |e| e.to_i }
    $screen[row].rotate!(-distance)
  elsif instruction.start_with?('rotate column')
    col, distance = instruction.scan(/\d+/).map { |e| e.to_i }
    $screen = $screen.transpose
    $screen[col].rotate!(-distance)
    $screen = $screen.transpose
  end
end

$rows.times { |r| $columns.times { |c| light_count += 1 if $screen[r][c] == '#' } }

puts "Part 1: #{light_count}"
puts "Part 2:"
print_screen()
