...
def find_keys(input, amount_of_keys, stretch)
  possible_keys = Hash.new
  confirmed_keys = []
  index = 0

  while confirmed_keys.length < amount_of_keys + 5 # keys may not be confirmed in order, so need to go beyond 64.
    md5 = Digest::MD5.hexdigest(input + index.to_s)
    stretch.times do md5 = Digest::MD5.hexdigest(md5) end

    possible_keys.keys.each do |k|
      five_in_a_row = /(#{possible_keys[k]})\1{4,}/
      confirmed_keys << k if md5 =~ five_in_a_row
    end

    confirmed_keys.each { |k| possible_keys.delete(k) }
    possible_keys.delete_if { |k,v| k + 1000 < index }

    three_in_a_row = /(.)\1{2,}/
    possible_keys[index] = md5[md5 =~ three_in_a_row] if md5 =~ three_in_a_row
...
puts "Part 1: #{find_keys(input, 64, 0).sort[63]}"
puts "Part 2: #{find_keys(input, 64, 2016).sort[63]}"
