...
# a marker is added to the file, like (10x2). To decompress this marker, take the subsequent 10 characters and repeat them
# 2 times. Then, continue reading the file after the repeated data. The marker itself is not included in the decompressed
# output.
#
...
# X(8x2)(3x3)ABCY becomes X(3x3)ABC(3x3)ABCY (for a decompressed length of 18), because the decompressed data from the (8x2)
...
while i < chars.length
  if chars[i] == '('
    len, repeat = chars[i+1, chars.index(')', i)-i-1].split('x').map(&:to_i)
    part1_length += len * repeat
    i = chars.index(')', i) + 1 + len
  else
    part1_length += 1
    i += 1
  end
...
# Part 2
weights = Array.new(chars.length).map { |e| e = 1 }
for i in 0..chars.length
  if chars[i] == '('
    end_marker = chars.index(')', i)
    len, repeat = chars[i+1, chars.index(')', i)-i-1].split('x').map(&:to_i)
    for j in i..end_marker
      weights[j] = 0
    end
    for j in end_marker+1..end_marker+len
      weights[j] *= repeat if j < weights.length
    end
  end
end

weights.each { |w| part2_length += w }

puts "Part 1: #{part1_length}"
puts "Part 2: #{part2_length}"
